...
interface OwnerOperatorSignup4MobileProps {
  onNext: () => void;
  onSkip: () => void;
  onBack: () => void;
}
...
const INSTITUTION_LISTS: Record<string, string[]> = {
  banco: ['Banco Atlas S.A.', 'Banco BASA S.A.', 'Banco Continental S.A.E.C.A.', 'Banco Familiar S.A.E.C.A.', 'Banco GNB Paraguay S.A.E.C.A.', 'Banco Itaú Paraguay S.A.', 'Banco Nacional de Fomento', 'Banco Regional S.A.E.C.A.', 'Sudameris Bank S.A.E.C.A.', 'Visión Banco S.A.E.C.A.', 'Ueno Bank S.A.'],
...
export function OwnerOperatorSignup4PaymentMobile({ onNext, onSkip, onBack }: OwnerOperatorSignup4MobileProps) {
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: '#F6F1E8' }}>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-6 pb-4">
        ...
        <form id="oo4-form" onSubmit={handleSubmit} className="space-y-5">
          <div>
              ...
              {TYPE_OPTIONS.map(({ id, icon: Icon, label }) => {
                ...
                return (
                  <button
                    key={id}
                    type="button"
                    onClick={() => { setSelectedType(id); setFormData({ ...formData, institution: '' }); }}
                    className="flex flex-col items-center justify-center py-4 rounded-2xl border-2 transition-all"
                    style={{ borderColor: sel ? '#1E5126' : '#e5e7eb', backgroundColor: sel ? '#f0fdf4' : 'white' }}
                  >
          ...
          {selectedType && (
            <>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-1.5">Nombre del titular *</label>
                <input
                  type="text"
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
                <p className="text-sm text-blue-900">TROPEX remitirá los pagos del ganadero directamente a esta cuenta.</p>
              </div>
            </>
          )}
        </form>
