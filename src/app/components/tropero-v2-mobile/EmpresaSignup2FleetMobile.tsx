...
interface EmpresaSignup2MobileProps {
...
export function EmpresaSignup2FleetMobile({ onNext, onSkip, onBack }: EmpresaSignup2MobileProps) {
  ...
  const addUnit = (typeId: string, typeName: string) => {
    ...
    setUnits([...units, { id: `${typeId}-${Date.now()}`, typeId, typeName, unitNumber: existing.length + 1, capacityAdult: 0, capacityYoung: 0 }]);
  };
  ...
  const totals = { units: units.length, adults: units.reduce((s, u) => s + u.capacityAdult, 0), young: units.reduce((s, u) => s + u.capacityYoung, 0) };
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: '#F6F1E8' }}>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-6 pb-4">
            ...
            return (
              <div key={type.id}>
                <button
                  ...
                  style={{ borderColor: selected ? '#1E5126' : '#e5e7eb', backgroundColor: selected ? '#f0fdf4' : 'white' }}
                >
                  <div className="flex items-center justify-center mb-2" style={{ height: 36 }}>
                    <TruckTypeIcon type={type.id} height={34} />
                  </div>
                  {selected && (
                    <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-white text-xs font-bold mb-1" style={{ backgroundColor: '#1E5126' }}>{count}</span>
                  )}
                ...
                {selected && (
                  <div className="flex items-center gap-2 mt-2 justify-center">
                    <button type="button" onClick={() => removeUnit(type.id)} className="flex items-center gap-1 px-3 py-2 border-2 rounded-xl text-xs font-semibold" style={{ borderColor: '#1E5126', color: '#1E5126' }}>
                      <Minus className="w-3.5 h-3.5" />
                    </button>
                    <button type="button" onClick={() => addUnit(type.id, type.name)} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-semibold text-white" style={{ backgroundColor: '#1E5126' }}>
            ...
            {units.map((unit) => (
              <div key={unit.id} className="bg-white rounded-2xl border border-gray-200 p-4">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: '#1E5126' }}>{unit.unitNumber}</div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" min="0" max="45" value={unit.capacityAdult || ''} onChange={(e) => updateUnit(unit.id, 'capacityAdult', Math.max(0, Math.min(45, parseInt(e.target.value) || 0)))} placeholder="Ej: 40" className="w-full px-3 py-3 text-sm border border-gray-300 rounded-xl bg-white focus:outline-none focus:border-[#1E5126]" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1.5">Jóvenes *</label>
                    <input type="number" min="0" max="80" value={unit.capacityYoung || ''} onChange={(e) => updateUnit(unit.id, 'capacityYoung', Math.max(0, Math.min(80, parseInt(e.target.value) || 0)))} placeholder="Ej: 70" className="w-full px-3 py-3 text-sm border border-gray-300 rounded-xl bg-white focus:outline-none focus:border-[#1E5126]" />
                  </div>
                </div>
              </div>
            ))}
        ...
        {units.length > 0 && (
          <div className="rounded-2xl p-4 border-2 mb-4" style={{ backgroundColor: '#f0fdf4', borderColor: '#1E5126' }}>
            <h3 className="font-bold text-gray-900 mb-3">Resumen de flota</h3>
            <div className="grid grid-cols-3 gap-2 text-center">
              {[{ label: 'Unidades', value: totals.units }, { label: 'Adultos', value: totals.adults }, { label: 'Jóvenes', value: totals.young }].map(({ label, value }) => (
                <div key={label}>
      ...
      <div className="px-5 pt-4 pb-8 bg-white border-t border-gray-100">
        <button
          onClick={() => { if (isValid) onNext({ units }); }}
          disabled={!isValid}
