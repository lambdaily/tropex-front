...
interface OwnerOperatorSignup5MobileProps {
  onNext: () => void;
  onSkip: () => void;
  onBack: () => void;
}

export function OwnerOperatorSignup5PayoutMethodMobile({ onNext, onSkip, onBack }: OwnerOperatorSignup5MobileProps) {
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: '#F6F1E8' }}>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-6 pb-4">
        ...
        <form id="oo5-form" onSubmit={handleSubmit} className="space-y-4 mb-6">
          <button
            type="button"
            onClick={() => setMethod('delayed')}
            className="w-full text-left border-2 rounded-2xl p-5 transition-all"
            style={{ borderColor: method === 'delayed' ? '#1E5126' : '#e5e7eb', backgroundColor: method === 'delayed' ? '#f0fdf4' : 'white' }}
          >
          ...
          <button
            ...
            className="w-full text-left border-2 rounded-2xl p-5 transition-all"
            style={{ borderColor: method === 'instant' ? '#d97706' : '#e5e7eb', backgroundColor: method === 'instant' ? '#fffbeb' : 'white' }}
          >
            <div className="flex items-center gap-3 mb-2">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
          <p className="text-sm text-blue-800">
            Podés cambiar esta preferencia en cualquier momento desde la configuración.
          </p>
        </div>
      </div>

      <div className="px-5 pt-4 pb-8 bg-white border-t border-gray-100 space-y-3">
        <button type="submit" form="oo5-form" className="w-full py-4 text-white font-bold text-base rounded-2xl" style={{ backgroundColor: '#1E5126' }}>
          Finalizar registro
        </button>
