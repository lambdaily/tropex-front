...
export function NewShipmentFlow({ onClose }: NewShipmentFlowProps) {
  ...
  const handleSubmit = () => {
    ...
    const guides = isSplit
      ? [
          { guideNumber: 1 as const, heads: Math.min(parseInt(formData.guide1Heads) || 0, limit), status: 'available' as const },
          { guideNumber: 2 as const, heads: parseInt(formData.guide2Heads) || 0, status: 'available' as const },
        ]
      : [{ guideNumber: 1 as const, heads: count, status: 'available' as const }];

    const distance = DEMO_PRICING.defaultDistanceKm;
    const marketPrice = estimateReferencePrice(count, distance);

    addOrder({
      rancherName: 'Demo Ganadero',
      ...
      cattleType: formData.cattleType as 'weaned' | 'fat',
      cattleTypeLabel: formData.cattleType === 'weaned' ? 'Desmamantes' : 'Gordos',
      heads: count,
      ...
      guides,
      status: 'new',
      estimatedWeight: parseInt(formData.estimatedWeight) || 380,
    });
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Paso {step} de {totalSteps}</p>
            </div>
          </div>
                    ...
                    onValueChange={(value) => {
                      if (value === 'mi-establecimiento') {
                        ...
                        setFormData({
                          ...
                          originLocation: profile?.establishmentName || 'Punta Brava',
                          originDepartment: profile?.establishmentAddress || 'PDTE. HAYES',
                        });
                      } else {
                        setFormData({
                          ...formData,
          ...
          {step === 2 && (
            <div className="bg-white rounded-lg border border-gray-200 p-6 lg:p-8" style={{ borderTop: '3px solid #1E5126' }}>
              ...
              <div className="space-y-5">
                    ...
                    onValueChange={(value) => {
                      if (value === 'my-establishment') {
                        ...
                        setFormData({
                          ...
                          destinationLocation: profile?.establishmentName || 'Punta Brava',
                        });
                      } else {
                        setFormData({
                          ...formData,
                ...
                {/* Ubicación de destino - Show autocomplete for frigorifico, regular input for others */}
      ...
      <div className="bg-white px-4 lg:px-6 py-4" style={{ borderTop: '2px solid #1E5126' }}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {step < totalSteps ? (
            ...
            <Button onClick={handleSubmit} style={{ backgroundColor: '#1E5126' }}>
