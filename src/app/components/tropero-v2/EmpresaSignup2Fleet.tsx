...
import { setEmpresaFleet, EmpresaVehicle } from '../../store/demoStore';

import type { SignupData } from '../../types/signup';
import { SignupHeader } from './SignupHeader';
interface EmpresaSignup2FleetProps {
...
export function EmpresaSignup2Fleet({ onNext, onSkip, onBack }: EmpresaSignup2FleetProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isValid()) {
      const vehiclePayload: EmpresaVehicle[] = units.map((unit, idx) => ({
        id: `CAM-R${String(idx + 1).padStart(3, '0')}`,
        plate: `REG-${String(idx + 1).padStart(3, '0')}`,
        type: TYPE_MAP[unit.typeId] || unit.typeId,
        capacityAdults: unit.capacityAdult,
        capacityYoung: unit.capacityYoung,
        status: 'disponible' as const,
        driverId: null,
      }));
      setEmpresaFleet(vehiclePayload);
      onNext({ units });
    }
  };
